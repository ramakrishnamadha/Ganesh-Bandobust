...
internal class AndroidARView(
    ...
    // Centre hit-tests must run against the fresh Frame returned by Session.update()
    // on the GL rendering thread. Keeping the MethodChannel Result lets us queue
    // the request and complete it on the next rendered frame.
    private val centerHitLock = Any()
    private var queuedCenterHitResult: MethodChannel.Result? = null
                    ...
                    when (call.method) {
                        ...
                        "hitTestCenter" -> {
                            ...
                            if (width <= 0 || height <= 0) {
                                result.error(
                                    "AR_VIEW_NOT_READY",
                                    "AR view size is not ready",
                                    null
                                )
                                return
                            }

                            synchronized(centerHitLock) {
                                // Avoid leaving an earlier Flutter call waiting forever.
                                queuedCenterHitResult?.error(
                                    "CENTER_HIT_REPLACED",
                                    "A newer centre hit-test request replaced the previous request",
                                    null
                                )
                                queuedCenterHitResult = result
                            }
                        }
                        "snapshot" -> {
                            val width = glSurfaceView.width
...
//                        onDestroy()
    ...
    fun onDestroy() {
        try {
            synchronized(centerHitLock) {
                queuedCenterHitResult?.error(
                    "AR_DISPOSED",
                    "AR view was disposed before the centre hit-test completed",
                    null
                )
                queuedCenterHitResult = null
            }
    ...
    private fun handleQueuedCenterHit(frame: Frame) {
        val pendingResult = synchronized(centerHitLock) {
            val value = queuedCenterHitResult
            queuedCenterHitResult = null
            value
        } ?: return
        ...
        try {
            if (frame.camera.trackingState != TrackingState.TRACKING) {
                activity.runOnUiThread {
                    pendingResult.success(ArrayList<HashMap<String, Any>>())
                }
                return
            }
            ...
            activity.runOnUiThread {
                pendingResult.success(serializedResults)
            }
        } catch (e: Exception) {
            activity.runOnUiThread {
                pendingResult.error(
                    "CENTER_HIT_TEST_ERROR",
                    e.message,
                    null
                )
            }
