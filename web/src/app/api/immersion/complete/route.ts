...
export async function POST(request: NextRequest) {
  try {
    ...
    // Since mobile sends it flat, we extract everything except some common keys as the data.
    const { gpid: _gpid, checkSource, userSessionId, deviceSessionId, checkStartedAt: _csa, checkedAt: _ca, ...resultData } = body;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.immersionCheckVisit.update({
        ...
        data: {
          currentStep: 6,
          finalConfirmationResult: resultData as any,
          status: "COMPLETED",
          updatedAt: new Date(),
      ...
      await tx.activityEvent.create({
        data: {
          ...
          eventType: "IMMERSION_COMPLETED",
          metadata: { gpid, step: 6, result: resultData } as any,
          occurredAt: new Date()
        }
      });
    });
    ...
    console.error("Immersion complete POST error:", error);
